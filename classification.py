...
for i in range(0, df_train.shape[0] - window_size, step_size):
    xs = df_train['x-axis'].values[i: i + 500]
    ys = df_train['y-axis'].values[i: i + 500]
    zs = df_train['z-axis'].values[i: i + 500]
    label = stats.mode(df_train['activity'][i: i + 500])[0][0]
    # print(label)
...
X_train['sma'] =    pd.Series(x_list).apply(lambda x: np.sum(abs(x)/100)) + pd.Series(y_list).apply(lambda x: np.sum(abs(x)/10)) + pd.Series(z_list).apply(lambda x: np.sum(abs(x)/100))
...
X_train['sma_fft'] = pd.Series(x_list_fft).apply(lambda x: np.sum(abs(x)/50)) + pd.Series(y_list_fft).apply(lambda x: np.sum(abs(x)/50)) + pd.Series(z_list_fft).apply(lambda x: np.sum(abs(x)/50))
...
for i in range(0, df_test.shape[0] - window_size, step_size):
    xs = df_test['x-axis'].values[i: i + 500]
    ys = df_test['y-axis'].values[i: i + 500]
    zs = df_test['z-axis'].values[i: i + 500]
    label = stats.mode(df_test['activity'][i: i + 500])[0][0]
...
# signal magnitude area
X_test['sma'] =    pd.Series(x_list).apply(lambda x: np.sum(abs(x)/100)) + pd.Series(y_list).apply(lambda x: np.sum(abs(x)/10)) + pd.Series(z_list).apply(lambda x: np.sum(abs(x)/100))
...
# FFT Signal magnitude area
X_test['sma_fft'] = pd.Series(x_list_fft).apply(lambda x: np.sum(abs(x)/50)) + pd.Series(y_list_fft).apply(lambda x: np.sum(abs(x)/50)) + pd.Series(z_list_fft).apply(lambda x: np.sum(abs(x)/50))
...
print("Activity- ",stats.mode(y_pred)[0])
